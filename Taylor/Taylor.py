...
with open("Taylor/intents.json") as file:
    data = json.load(file)

try:
    with open("taylor.pickle", "rb") as file:
    ...
    with open("taylor.pickle", "wb") as file:
        pickle.dump((words, labels, training, output), file)

try:
    json_file = open('taylor.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    myChatModel = model_from_json(loaded_model_json)
    myChatModel.load_weights("taylor.h5")
    ...
    with open("taylor.json", "w") as json_file:
        json_file.write(model_json)

    # serialize weights to HDF5
    myChatModel.save_weights("taylor.h5")
