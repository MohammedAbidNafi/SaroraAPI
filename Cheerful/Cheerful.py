...
with open("Cheerful/intents.json") as file:
    data = json.load(file)

try:
    with open("cheerful.pickle", "rb") as file:
    ...
    with open("cheerful.pickle", "wb") as file:
        pickle.dump((words, labels, training, output), file)

try:
    json_file = open('cheerful.json', 'r')
    loaded_model_json = json_file.read()
    json_file.close()
    myChatModel = model_from_json(loaded_model_json)
    myChatModel.load_weights("cheerful.h5")
    ...
    with open("cheerful.json", "w") as json_file:
        json_file.write(model_json)

    # serialize weights to HDF5
    myChatModel.save_weights("cheerful.h5")
...
def chatWithCheerful(inputText):
